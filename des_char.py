...
char_xor= int('0x0020000800000400',0) #input XOR for S boxes after initial permutation
...
for i in range(1600): #trying for 1600 inputs
	...
	try:
		...
		if(len(out2)<20):
			...
			#print(last_out_xor)
			s_out_xor=inv_s_permut(bin(int('0x00000400',0)^int('00001110000100110000110100100101',2)^int(last_out_xor,0))) #computing output XOR for S boxes
			#print((ex_in_1))
			#print((ex_in_2))
			#print((ex_in_xor))
			...
			ex_in=[]  #input to S1,S2,S4,S5,S6 boxes
			ex_in.append('0b'+ex_in_1[2:8])
			ex_in.append('0b'+ex_in_1[8:14])
			ex_in.append('0b'+ex_in_1[20:26])
			ex_in.append('0b'+ex_in_1[26:32])
			ex_in.append('0b'+ex_in_1[32:38])

			ex_in_x=[] #input XOR to S1,S2,S4,S5,S6 boxes
			ex_in_x.append('0b'+ex_in_xor[2:8])
			ex_in_x.append('0b'+ex_in_xor[8:14])
			ex_in_x.append('0b'+ex_in_xor[20:26])
			ex_in_x.append('0b'+ex_in_xor[26:32])
			ex_in_x.append('0b'+ex_in_xor[32:38])

			s_out=[] #output XOR to S1,S2,S4,S5,S6 boxes
			s_out.append('0b'+s_out_xor[2:6])
			s_out.append('0b'+s_out_xor[6:10])
			s_out.append('0b'+s_out_xor[14:18])
			...
			#print(ex_in)
			k=0
			for itr in (0,1,3,4,5):
				lst=xor_table[itr][int(ex_in_x[k],0)][int(s_out[k],0)]
				for val in lst:
